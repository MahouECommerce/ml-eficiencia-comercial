...
#
order_detail_sorted['TotalOnlinePurchases'] = order_detail_sorted[order_detail_sorted['Origin']
                                                                  == 'Online'].groupby('PointOfSaleId')['Origin'].transform('count')
order_detail_sorted['TotalOfflinePurchases'] = order_detail_sorted[order_detail_sorted['Origin']
                                                                   != 'Online'].groupby('PointOfSaleId')['Origin'].transform('count')

#
order_detail_sorted['TotalOnlinePurchases'] = order_detail_sorted['TotalOnlinePurchases'].fillna(
    0)
order_detail_sorted['TotalOfflinePurchases'] = order_detail_sorted['TotalOfflinePurchases'].fillna(
    0)
...
order_detail_sorted = order_detail_sorted.drop_duplicates(
    subset=['PointOfSaleId', 'NameDistributor']).reset_index(drop=True)

print('fin')

# Segmento de CLIENTES OFFLINE
# Filtrar los clientes que tienen todas sus compras offline
clientes_solo_offline = order_detail_sorted.groupby(
    'PointOfSaleId').filter(lambda x: (x['Origin'] != 'Online').all())

pdvs_offline = clientes_solo_offline['PointOfSaleId'].unique().tolist()

order_detail_sorted = order_detail_sorted[~order_detail_sorted['PointOfSaleId'].isin(
    pdvs_offline)]
...
last_online_order_dates = \
    online_orders[['PointOfSaleId', "DaysSinceLastOnlinePurchase"]]
...
###### Análisis de Clientes Espejo por Distribuidor, Sellout, Frecuencia Total


# Resumen de distribución de clusters por distribuidor
cluster_por_distribuidor = order_detail_sorted.groupby(['Cluster', 'NameDistributor'])['PointOfSaleId'].count().unstack(fill_value=0)
print(cluster_por_distribuidor)


# Perfilado 
perfil_cluster = order_detail_sorted.groupby('Cluster')[['FrequencyTotal', 'Sellout', 'NumeroReferenciasUnicas']].agg(['mean', 'median', 'std']).reset_index()
...
# clientes espejo potenciales
clientes_espejo = order_detail_sorted[
    (order_detail_sorted['Cluster'].isin(clusters_optimos['Cluster'])) &
    (order_detail_sorted['Origin'] != 'Online')
]


print(clientes_espejo[['PointOfSaleId', 'Cluster', 'NameDistributor', 'FrequencyTotal', 'Sellout', 'NumeroReferenciasUnicas']])
